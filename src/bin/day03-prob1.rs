...
fn main() {
    ...
    let adjacent = get_adjacent_part_numbers(part_nums, symbols);
    println!(
        "{:?}",
        adjacent.into_iter().map(|val| val as u32).sum::<u32>()
    );
}
...
.664.598.."
    ...
    fn test_get_adjacent_part_numbers() {
        assert_eq!(
            ...
            vec![35, 467, 592, 598, 617, 633, 664, 755]
        )
    }
}
